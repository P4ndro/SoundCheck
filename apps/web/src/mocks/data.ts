import { MOCK_BAND_ID } from "@/mocks/constants";
import type { BandWorkspace } from "@/types";
import {
  ironCurtainGuitarTab,
  neonCathedralBassTab,
  neonCathedralChordChart,
  neonCathedralDrumChart,
  neonCathedralDrumsTab,
  velvetStaticBassTab,
} from "@/mocks/tab-content";
import { joinTabSections, measure, tabLine, tabSection } from "@/lib/tab-format";

const e = () => measure("");
const b4 = (n: number) => Array(n).fill("").map(() => e());

const midnightRelayBassTab = joinTabSections([
  tabSection("Intro", [
    tabLine("G", b4(4)),
    tabLine("D", Array(4).fill(measure("--7---7---7-"))),
    tabLine("A", Array(4).fill(measure("--5---5---5-"))),
    tabLine("E", b4(4)),
  ]),
  tabSection("Verse", [
    tabLine("G", b4(4)),
    tabLine("D", Array(4).fill(measure("--5---5---5-"))),
    tabLine("A", Array(4).fill(measure("--3---3---3-"))),
    tabLine("E", b4(4)),
  ]),
  tabSection("Chorus", [
    tabLine("G", b4(4)),
    tabLine("D", Array(4).fill(measure("--7---7---7-"))),
    tabLine("A", Array(4).fill(measure("--5---5---5-"))),
    tabLine("E", b4(4)),
  ]),
  tabSection("Bridge", [
    tabLine("G", b4(2)),
    tabLine("D", [measure("--3---5---7-"), measure("--5---3---1-")]),
    tabLine("A", [measure("--1---3---5-"), measure("--3---1-------")]),
    tabLine("E", b4(2)),
  ]),
  tabSection("Outro", [
    tabLine("G", b4(2)),
    tabLine("D", [measure("--7---5---3-"), e()]),
    tabLine("A", [measure("--5-----------"), e()]),
    tabLine("E", b4(2)),
  ]),
]);

const BAND_ID = MOCK_BAND_ID;
const NOW = "2026-06-01T10:00:00.000Z";

export const initialWorkspace: BandWorkspace = {
  currentUser: {
    id: "user-alex",
    name: "Alex Mercer",
    email: "alex@example.com",
  },
  band: {
    id: BAND_ID,
    name: "The Marlowe",
    createdAt: "2025-09-14T10:00:00.000Z",
  },
  users: [
    { id: "user-alex", name: "Alex Mercer", email: "alex@example.com" },
    { id: "user-jamie", name: "Jamie Cole", email: "jamie@example.com" },
    { id: "user-sam", name: "Sam Rivera", email: "sam@example.com" },
    { id: "user-riley", name: "Riley Park", email: "riley@example.com" },
  ],
  members: [
    {
      id: "member-1",
      userId: "user-alex",
      bandId: BAND_ID,
      role: "bass",
      joinedAt: "2025-09-14T10:00:00.000Z",
    },
    {
      id: "member-2",
      userId: "user-jamie",
      bandId: BAND_ID,
      role: "drums",
      joinedAt: "2025-09-14T10:00:00.000Z",
    },
    {
      id: "member-3",
      userId: "user-sam",
      bandId: BAND_ID,
      role: "vocals",
      joinedAt: "2025-10-02T10:00:00.000Z",
    },
    {
      id: "member-4",
      userId: "user-riley",
      bandId: BAND_ID,
      role: "lead_guitar",
      joinedAt: "2025-10-02T10:00:00.000Z",
    },
  ],
  songs: [
    {
      id: "song-1",
      bandId: BAND_ID,
      title: "Neon Cathedral",
      bpm: 128,
      timeSignature: "4/4",
      durationSeconds: 245,
      status: "in_progress",
      key: "Am",
      tuning: "E A D G",
      lyrics:
        "Flicker in the neon cathedral\nWe built our ghosts from borrowed light\nEvery echo tells a story\nEvery shadow knows the night",
      notes: "Bridge needs tighter stops. Watch the tempo drop at bar 17.",
      createdAt: "2025-11-01T10:00:00.000Z",
      updatedAt: "2026-05-28T14:30:00.000Z",
    },
    {
      id: "song-2",
      bandId: BAND_ID,
      title: "Slow Burn",
      bpm: 92,
      timeSignature: "3/4",
      durationSeconds: 312,
      status: "not_started",
      key: "",
      tuning: "",
      lyrics:
        "Take your time with me\nLet the embers breathe\nNothing here is urgent\nExcept the way you leave",
      notes: "Sam wants to try a lower key for the chorus.",
      createdAt: "2026-01-10T10:00:00.000Z",
      updatedAt: "2026-01-10T10:00:00.000Z",
    },
    {
      id: "song-3",
      bandId: BAND_ID,
      title: "Velvet Static",
      bpm: 110,
      timeSignature: "4/4",
      durationSeconds: 198,
      status: "completed",
      key: "C",
      tuning: "E A D G",
      lyrics:
        "Velvet static on the radio\nTuning out the world below\nHold the feedback like a secret\nOnly we were meant to know",
      notes: "Performance-ready. Setlist staple.",
      createdAt: "2025-08-20T10:00:00.000Z",
      updatedAt: "2026-04-12T09:00:00.000Z",
    },
    {
      id: "song-4",
      bandId: BAND_ID,
      title: "Midnight Relay",
      bpm: 140,
      timeSignature: "4/4",
      durationSeconds: 220,
      status: "instrumental_ready",
      key: "Em",
      tuning: "E A D G",
      lyrics:
        "Run the midnight relay\nPass the signal down the line\nCity lights like Morse code\nYou're the pulse and you're the sign",
      notes: "Instrumental sections locked. Vocals still being refined.",
      createdAt: "2025-12-05T10:00:00.000Z",
      updatedAt: "2026-05-20T11:00:00.000Z",
    },
    {
      id: "song-5",
      bandId: BAND_ID,
      title: "Hollow Frequency",
      bpm: 118,
      timeSignature: "6/8",
      durationSeconds: 267,
      status: "in_progress",
      key: "Dm",
      tuning: "E A D G",
      lyrics:
        "Tuned to a hollow frequency\nStatic where your voice should be\nI keep calling through the feedback\nBut the line won't answer me",
      notes: "Bass groove is set. Riley working on second guitar layer.",
      createdAt: "2026-02-18T10:00:00.000Z",
      updatedAt: "2026-05-30T16:45:00.000Z",
    },
    {
      id: "song-6",
      bandId: BAND_ID,
      title: "Glass Harbour",
      bpm: 85,
      timeSignature: "3/4",
      durationSeconds: 340,
      status: "not_started",
      key: "",
      tuning: "",
      lyrics:
        "Meet me at the glass harbour\nWhere the tide forgets the shore\nEvery vessel leaves a whisper\nEvery whisper asks for more",
      notes: "Ballad — save for encore slot.",
      createdAt: "2026-03-22T10:00:00.000Z",
      updatedAt: "2026-03-22T10:00:00.000Z",
    },
    {
      id: "song-7",
      bandId: BAND_ID,
      title: "Iron Violet",
      bpm: 132,
      timeSignature: "4/4",
      durationSeconds: 186,
      status: "completed",
      key: "E minor",
      tuning: "E A D G B E",
      lyrics:
        "Iron violet in the rain\nForged and bent but not undone\nStrike the anvil with conviction\nRise again into the sun",
      notes: "High energy opener. Confirmed for Friday gig.",
      createdAt: "2025-07-11T10:00:00.000Z",
      updatedAt: "2026-05-15T08:00:00.000Z",
    },
    {
      id: "song-8",
      bandId: BAND_ID,
      title: "Paper Lanterns",
      bpm: 96,
      timeSignature: "6/8",
      durationSeconds: 255,
      status: "instrumental_ready",
      key: "Am",
      tuning: "",
      lyrics:
        "Paper lanterns in the courtyard\nFloating up in borrowed flame\nEvery wish we never whispered\nFinds a light and finds a name",
      notes: "Great for rehearsal warm-up. Full band arrangement done.",
      createdAt: "2026-04-01T10:00:00.000Z",
      updatedAt: "2026-05-25T13:20:00.000Z",
    },
  ],
  tabs: [
    {
      id: "tab-1",
      songId: "song-1",
      instrument: "bass",
      asciiTab: neonCathedralBassTab,
      chordChart: neonCathedralChordChart,
    },
    {
      id: "tab-2",
      songId: "song-1",
      instrument: "drums",
      asciiTab: neonCathedralDrumsTab,
      chordChart: neonCathedralDrumChart,
    },
    {
      id: "tab-3",
      songId: "song-3",
      instrument: "bass",
      asciiTab: velvetStaticBassTab,
      chordChart:
        "Intro: C\nVerse: C  Am  F  G\nChorus: F  G  Am  C\nBridge: F  Am  G  C\nOutro: C",
    },
    {
      id: "tab-4",
      songId: "song-4",
      instrument: "bass",
      asciiTab: midnightRelayBassTab,
      chordChart:
        "Intro: Em  D  C  D\nVerse: Em  D  C  D\nChorus: G  D  Em  C\nBridge: C  D  Em  G\nOutro: Em",
    },
    {
      id: "tab-5",
      songId: "song-5",
      instrument: "bass",
      asciiTab: joinTabSections([
        tabSection("Intro", [
          tabLine("G", b4(2)),
          tabLine("D", Array(2).fill(measure("----3---3---"))),
          tabLine("A", Array(2).fill(measure("--1---1---1-"))),
          tabLine("E", b4(2)),
        ]),
        tabSection("Verse", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("----5---5---"))),
          tabLine("A", Array(4).fill(measure("--3---3---3-"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Chorus", [
          tabLine("G", b4(4)),
          tabLine("D", [
            measure("----7---5---"),
            measure("----7---5---"),
            measure("----5---3---"),
            measure("----5---3---"),
          ]),
          tabLine("A", [
            measure("--5-----------"),
            measure("--5-----------"),
            measure("--3-----------"),
            measure("--3-----------"),
          ]),
          tabLine("E", b4(4)),
        ]),
        tabSection("Bridge", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("----3---5---"), measure("----5---3---")]),
          tabLine("A", [measure("--1---3---4-"), measure("--3---1-------")]),
          tabLine("E", b4(2)),
        ]),
        tabSection("Outro", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("----3-----------"), measure("----1-----------")]),
          tabLine("A", [measure("--1-------------"), e()]),
          tabLine("E", b4(2)),
        ]),
      ]),
      chordChart:
        "Intro: Dm\nVerse: Dm  Bb  F  C\nChorus: Bb  F  C  Dm\nBridge: Gm  A  Bb  C\nOutro: Dm",
    },
    {
      id: "tab-6",
      songId: "song-7",
      instrument: "bass",
      asciiTab: joinTabSections([
        tabSection("Intro", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("--7---7---7-"))),
          tabLine("A", Array(4).fill(measure("--5---5---5-"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Verse", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("--3---5---7-"))),
          tabLine("A", Array(4).fill(measure("--1---3---5-"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Chorus", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("--7---5---3-"))),
          tabLine("A", Array(4).fill(measure("--5---3---1-"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Bridge", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("--5---3---1-"), measure("--3---5---7-")]),
          tabLine("A", [measure("--3---1-------"), measure("--1---3---5-")]),
          tabLine("E", b4(2)),
        ]),
        tabSection("Outro", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("--7---5---3-"), e()]),
          tabLine("A", b4(2)),
          tabLine("E", b4(2)),
        ]),
      ]),
      chordChart:
        "Intro: E5  G5  A5  B5\nVerse: E5  G5  A5  B5\nChorus: E5  D5  C5  B5\nBridge: C5  B5  A5  G5\nOutro: E5",
    },
    {
      id: "tab-7",
      songId: "song-7",
      instrument: "lead_guitar",
      trackName: "Lead · distortion",
      capo: 2,
      asciiTab: ironCurtainGuitarTab,
      chordChart:
        "Intro: E minor pentatonic riff\nVerse: E5  G5  A5  B5\nChorus: E5  D5  C5  B5\nSolo: bars 45–61, bend +1 on 10th fret G string\nOutro: E5  B5",
    },
    {
      id: "tab-8",
      songId: "song-8",
      instrument: "bass",
      asciiTab: joinTabSections([
        tabSection("Intro", [
          tabLine("G", b4(2)),
          tabLine("D", Array(2).fill(measure("----1---1---"))),
          tabLine("A", Array(2).fill(measure("--3---3---3-"))),
          tabLine("E", b4(2)),
        ]),
        tabSection("Verse", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("----3---3---"))),
          tabLine("A", Array(4).fill(measure("--1---1---1-"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Chorus", [
          tabLine("G", b4(4)),
          tabLine("D", Array(4).fill(measure("----5---3---"))),
          tabLine("A", Array(4).fill(measure("--3-----------"))),
          tabLine("E", b4(4)),
        ]),
        tabSection("Bridge", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("----3---5---"), measure("----1---3---")]),
          tabLine("A", [measure("--1-----------"), measure("--3-----------")]),
          tabLine("E", b4(2)),
        ]),
        tabSection("Outro", [
          tabLine("G", b4(2)),
          tabLine("D", [measure("----1---1---"), e()]),
          tabLine("A", [measure("--3-----------"), measure("--1-------------")]),
          tabLine("E", b4(2)),
        ]),
      ]),
      chordChart:
        "Intro: Am\nVerse: Am  G  F  E\nChorus: F  C  G  Am\nBridge: Dm  G  C  Am\nOutro: Am",
    },
  ],
  setlists: [
    {
      id: "setlist-1",
      bandId: BAND_ID,
      name: "Friday Gig",
      description: "Main set for The Velvet Room — 45 min slot.",
      songIds: ["song-7", "song-3", "song-1", "song-4", "song-8"],
      createdAt: "2026-05-10T10:00:00.000Z",
      updatedAt: NOW,
    },
    {
      id: "setlist-2",
      bandId: BAND_ID,
      name: "Sunday Rehearsal",
      description: "Focus on new material and transitions.",
      songIds: ["song-1", "song-5", "song-2", "song-6"],
      createdAt: "2026-05-18T10:00:00.000Z",
      updatedAt: "2026-05-27T10:00:00.000Z",
    },
  ],
  events: [
    {
      id: "event-1",
      bandId: BAND_ID,
      title: "Friday Gig — The Velvet Room",
      type: "gig",
      start: "2026-06-13T19:00:00.000Z",
      end: "2026-06-13T22:00:00.000Z",
      location: "The Velvet Room, 42 Bridge Street",
      notes: "Load-in at 6 PM. Backline provided. Bring own bass cab.",
      setlistId: "setlist-1",
    },
    {
      id: "event-2",
      bandId: BAND_ID,
      title: "Sunday Rehearsal",
      type: "rehearsal",
      start: "2026-06-08T14:00:00.000Z",
      end: "2026-06-08T17:00:00.000Z",
      location: "Studio B, Riverside Arts",
      notes: "Run Friday set first, then new songs.",
      setlistId: "setlist-2",
    },
    {
      id: "event-3",
      bandId: BAND_ID,
      title: "Band Meeting — Summer Schedule",
      type: "meeting",
      start: "2026-06-15T18:30:00.000Z",
      end: "2026-06-15T19:30:00.000Z",
      location: "Riverside Café",
      notes: "Discuss July bookings and merch.",
    },
    {
      id: "event-4",
      bandId: BAND_ID,
      title: "Outdoor Festival Set",
      type: "gig",
      start: "2026-07-04T16:00:00.000Z",
      end: "2026-07-04T17:00:00.000Z",
      location: "Harbour Park Main Stage",
      notes: "30-minute slot. Tentative setlist TBD.",
    },
  ],
};
